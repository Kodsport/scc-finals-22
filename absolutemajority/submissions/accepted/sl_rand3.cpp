// Like sl_rand1 but the query graph for all the first Q-1 queries together is regular.
#include <bits/stdc++.h>
using namespace std;
...
int main() {
	...
	UF uf(N);
	int Q2 = Q-1;
	vi regTargets = genRegular(N*Q2);

	// Regenerate graph if multiple edges or loops, limiting retries because
	// it's inevitable if the graph is small enough
	rep(it,0,40) {
		map<pii, int> edges;
		rep(i,0,N) edges[pii(i, i)]++;
		rep(qi,0,Q2) rep(i,0,N) {
			int j = regTargets[i*Q2 + qi]/Q2;
			if (i == j) goto bad;
			pii ed(min(i, j), max(i, j));
			if (edges[ed]++ && it < 20) goto bad;
		}
		break;
bad:
		regTargets = genRegular(N*Q2);
	}

	rep(qi,0,Q2) {
		vi target(N);
		rep(i,0,N) target[i] = regTargets[i*Q2 + qi]/Q2;
		cout << "? ";
