/* @EXPECTED_GRADES@ TLE RTE TLE TLE TLE */
#include <bits/stdc++.h>
using namespace std;
...
bool dfs(bool checkCanCrash) {
    vector<vector<vector<vector<bool>>>> seen = make(false, W, H, W, H);
    queue<tuple<int, int, int, int>> Q;
    Q.emplace(xs1, ys1, xs2, ys2);
    seen[xs1][ys1][xs2][ys2] = true;
    ...
    while (!Q.empty()) {
        ...
        trav(p1, m1) trav(p2, m2) {
            ...
            if (seen[nxp1][nyp1][nxp2][nyp2]) continue;
            seen[nxp1][nyp1][nxp2][nyp2] = true;
            Q.emplace(nxp1, nyp1, nxp2, nyp2);
        }
    }
    if (checkCanCrash) return false;
    return seen[xd1][yd1][xd2][yd2];
}
