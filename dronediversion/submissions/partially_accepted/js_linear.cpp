...
void shift(pii& start, pii dest, int moves) {
    ...
    else if (start.second == dest.second) move(start.first, dest.first, moves);
    else assert(false && "shifting along two directions");
}
...
bool intersects(pair<pii, pii> a, pair<pii, pii> b, int ashift, int bshift) {
    ...
    int t = min(dist(a.first, a.second), dist(b.first, b.second));
    for (int i = 0; i < t; i++) {
        if (a.first == b.first) return true;
        shift(a.first, a.second, 1);
        shift(b.first, b.second, 1);
    }
    if (a.first == b.first) return true;
    return false;
}
